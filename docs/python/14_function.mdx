...
# 函式
一個有一定規模的程式系統背後，會是許多人一起共同開發，如何讓每個人設計的功能整合起來，<br/>
函式是一個非常好解決這件事情的方法之一。

## 執行函式
還記得 `print()`、`input()` 或是 `print()` 這幾個`方法`(指令)，
在執行時都會再後面加上一對小括號 `()` 表示執行，這些就是設計好的**函式**。

### 內建函式
預先設計好的函式我們稱為「內建函式」。我們並不需要知道 `print` 指令背後為什麼可以讓網頁跳出文字框，
我們只需要知道執行 `print` 函式可以做到這件事情就好。這就是函式的作用，將功能包裝起來，
讓使用者不需要了解細節就可以應用它。
...
我們可以使用 `def` 語法去創造函式，後面接著的是函式的名稱，執行時會使用到它。<br/>
再接著會有一對小括號 `()`。

小括號內是機器的原物料入口，定義這台機器需要輸入什麼樣的東西。<br/>
大括號內是機器主要運作功能，定義這台機器會如何運作來完成任務。<br/>
