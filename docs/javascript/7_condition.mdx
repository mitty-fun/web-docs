...
如果我們想要在成績不及格時，顯示「不及格」的文字訊息可以這麼做
<Playground javascript={`
let score = Number(prompt('成績：'));\n
if (score >= 60) {
    console.log('及格');
...
`}/>
再加上另外一個條件判斷，判斷說如果數值低於 60 就顯示不及格，
這樣所有輸入的數字範圍都可以涵蓋到。
...
並讓程式顯示「補考」該怎麼做呢？我們可以在小於 60 分時再加上一個條件判斷。
<Playground javascript={`
let score = Number(prompt('成績：'));\n
if (score >= 60) {
    console.log('及格');
...
如果你試著去執行程式，你會發現 A 範例會有錯誤，這種錯誤就是如果沒有好好排版容易導致的錯誤，你能發現嗎？

#### A. 沒有縮排的程式，不易閱讀，而且有語法錯誤。
<Playground javascript={`
if (score >= 60) {
console.log('及格');
} else {
...
為了解決多重條件的情境，因此程式提供了 `elif` 如果否則語法，
我們可以應用它將程式改寫成如下：
<Playground javascript={`
let score = Number(prompt('成績：'));\n
if (score >= 90) {
